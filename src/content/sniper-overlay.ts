...
function createPanel() {
    ...
    <div id="syndrax-sniper-inner">
      ...
      <div class="sniper-section" style="position:sticky;bottom:0;background:#02050f;padding-bottom:20px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <button id="sniper-lens-btn" class="sniper-btn-secondary">🔍 Lens</button>
        </div>
      </div>
    </div>
  `;
  ...
  // Google Lens button
  panel.querySelector('#sniper-lens-btn')?.addEventListener('click', () => {
    if (!currentData?.image) {
      showToast('❌ No image available', true);
      return;
    }
    window.open(`https://lens.google.com/uploadbyurl?url=${encodeURIComponent(currentData.image)}`, '_blank');
  });
